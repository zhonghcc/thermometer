...
As you can see, we've added a little bit of markup to the DOM. First, <tt>thermometer_current_status</tt> contains our title. By default, it will use the text from the currently active list item. You can override this behavior using a configuration option (see below). 
...
You can change the name of the class for currently active state:
  $('ul').thermometer({
...
== Demo.
Try out the {live demo}[http://coryschires.github.com/thermometer/].
